---
layout: post
title: "Eine Regionalbibliographie in Wikipedia sichtbar machen"
date: 2026-06-23
...
Dieser Beitrag beschreibt, wie wir mithilfe eines Wikipedia Bots automatisiert mehr als 4300 Verlinkungen in Wikipedia-Artikeln über Ortschaften in Nordrhein-Westfalen ergänzt haben. Diese Links zeigen auf Listen von Einträgen über den jeweiligen Ort in der [Nordrhein-Westfälischen Bibliographie (NWBib))](https://nwbib.de).

## Vorarbeiten: Von Textstrings zum SKOS Vokabular auf Wikidata-Basis

In einem NWBib-Vorgängerprojekt von 2017 bis 2020 wurden die Grundlagen geschaffen, indem wir die String-basierte Ortsverschlagwortung auf kontrollierte Werte umgestellt haben. Dabei hatten wir uns für Wikidata als zugrundeliegende Normdatenbank entschieden, aus dem wir die entsprechenden Ortseinträge extrahieren und in eine hierarchische Raumsystematik überführen.
...
* Diese Informationen aus Wikidata (plus ein paar weitere Systematikeinträge, die nicht aus Wikidata kommen) werden genommen, um die Raumsystematik in Form einer [SKOS-Datei](https://github.com/hbz/lobid-vocabs/blob/master/nwbib/nwbib-spatial.ttl)[^skos] zu generieren, die wiederum als Basis für die [Raumsystematik in der Webanwendung](https://nwbib.de/spatial) dient.
* Die Raumsystematik der NWBib lässt sich unter https://nwbib.de/spatial browsen und ermöglicht es, mit einem Klick auf die Literaturliste zu einem Ort zu kommen. Zudem basiert eine Filterfunktionen der NWBib-Suche auf der Systematik.
* Möchten NWBib-Redakteur:innen einen neue Systematikstelle ergänzen, dann fügen sie zum entsprechenden Wikidata-Eintrag ein P6814-Statement hinzu und wir lösen einen Neuaufbau der Systematik aus, damit der Ort auch in der SKOS-Datei und auf der Webseite erscheint.

## Das Ziel

Es gibt also seit Jahren eine Verknüpfung von Orten in der NWBib-Raumsystematik zu Wikidata und wir können uns in der NWBib eine Liste mit Literatur zu einem Ort auf Basis seiner Q-ID in Wikidata anzeigen lassen, z.B. Köln mit der Q-ID 365: https://nwbib.de/search?nwbibspatial=https%3A%2F%2Fnwbib.de%2Fspatial%23Q365

In Wikidata stehen wiederum Links auf den zugehörigen Artikel in der deutschsprachigen Wikipedia, wenn es denn einen gibt. Die Idee: Dieser Umstand ließe sich doch gut nutzen, um auch eine Verknüpfung der Wikipedia Einträge zu Orten in NRW mit der entsprechenden NWBib-Trefferliste herzustellen.

Die Idee der Wikipedia-NWBib-Verknüpfung kam mit der Umsetzung des Projekts und wurde dann im November 2025  reaktiviert im Kontext des [1. bundesweiten Treffen der Wikimedians in Bibliotheken](https://de.wikiversity.org/wiki/Wikimedians_in_Bibliotheken).

## ...zur Anreicherung von Wikipedia durch einen Bot

Da es sich um mehr als 4000 Wikipediartikel handel, wäre eine manuelle Ergänzung von Links ein sehr aufwändiges Unterfangen. Eine automatisierte Anreichung hingegen würde den Aufwand erheblich reduzieren.  Da wir selbst nicht so im Wikiversum so vertraut sind, haben wir über einen [Thema im metadaten.community-Forum](https://metadaten.community/t/links-in-wikipedia-artikeln-auf-wikidata-basis-automatisiert-ergaenzen/939) versucht uns Ratschläge aus der Bibliothekswelt zu holen.

Danke an [Christian Erlinger](https://librerli.eu/), der uns dabei geholfen hat, eine [Vorlage](https://de.wikipedia.org/wiki/Vorlage:NWBib.de) zu erstellen, damit nicht für jeden Link die inividualisierte URL eingefügt werden muss. Auf Basis dieser Vorlage muss nun einfach `{{NWBib.de}}` in einem Wikipedia-Artikel ergänzt werden, um die Verlinkung zu ergänzen. Im Hintergrund wird dabei auf die Informationen in Wikidata zugegriffen.
...
- https://de.wikipedia.org/wiki/Wikipedia:Bots/Antr%C3%A4ge_auf_Botflag
- https://de.wikipedia.org/wiki/Wikipedia:Bots
- https://www.mediawiki.org/wiki/Manual:Creating_a_bot/de


## PAWS to the rescue

Neulingen bieten diese Seiten zwar einen Einstieg, aber sie helfen einem nicht direkt dabei, einen Bot zu erstellen. Dazu kommt eine Vielzahl an Möglichkeiten und Frameworks, was die Umsetzung nicht leichter macht. Für uns hat das von Wikimedia bereitgestellte [PAWS](https://wikitech.wikimedia.org/wiki/PAWS) einiges erleichtert. PAWS ermöglicht das Erstellen von [Jupyter Notebooks](https://jupyter.org/), einer Programmierumgebung im Browser, innerhalb der ich mit dem eigenen Account direkt Daten aus dem Wikiverse abfragen, analysieren aber auch manipulieren kann.
...
Um die Sperre aufzuheben, mussten wir für den sperrenden Wikipedianer die [Bugs](https://de.wikipedia.org/wiki/Benutzer:NWBibBot/Bugs) beheben und zusagen, dass wir in kleineren Schritten Datensätze anpassen und die Anzahl der Änderungen erst ausweiten, sobald wir sichergehenkönnen, dass die Fehler nicht mehr auftreten. 

## Let it run

Mit Hilfe von Hinweisen der Wikipedianer liesen sich schnell die Bugs ausbessern. Dadurch konnten wir die Anzahl der Änderungen Schrittweise von 25 auf 1000 steigern. Die finale Version des Bots durchläuft folgende Schritte:

- SPQRQL-Abfrage an Wikipedia, nach allen Items, die eine NWBib-Id haben und eine deutschsprachige Wikipedia-Seite
- 1. Prüfung, wenn alles ja, dann weiter:
    ...
    - ist die Vorlage `{{NWBib.de}}` noch nicht eingebunden
- 2. Prüfung, entscheidet über eines vorgehen:
    - Abschnitt `[[Weblinks]]` existiert
        - ergänze Vorlage `{{NWBib.de}}` am Anfang der Weblink-Liste
    - Abschnitt `[[Einzelnachweise]]` existiert, aber kein Abschnitt `[[Weblinks]]`
        - ergänze`[[Weblinks]]` mit Vorlage `{{NWBib.de}}` vor dem Abschnitt `[[Einzelnachweise]]`
    -  weder `[[Einzelnachweise]]` noch Abschnitt `[[Weblinks]]` existiert
        -  ergänze `[[Weblinks]]` mit Vorlage `{{NWBib.de}}` am Ende des Artikels, aber vor der Navigationsleiste und Kategorien
...
Dadurch waren wir in der Lage aktuell 4309 Verlinkungen zur NWBib in den Wikipedia-Seiten zu ergänzen.

## Lessons learned

- Trotz vieler Doku ist der Einstieg in die automatisierte Bearbeitung von Wikipedia-Seiten nicht leicht, gerade auch bei pywikibot sind zwar alle Funktionen dokumentiert, Anwendungsfälle sind aber eher auf Wikidata bzw. auf die fertigen Scripte begrenzt. Dies kann aber auch auf begrenzte Python-Kenntnisse und wenig Kenntnisse formaler Regeln bei Wikipedia auf unserer Seite liegen.
...
- PAWS ist ein tolles Tool, leider fehlt die Möglichkeit für Versionierung der Jupyter-Notebooks. Dies hat es erschwert, die Entwicklungsschritte und kleine Änderungen beim Testen festzuhalten.
