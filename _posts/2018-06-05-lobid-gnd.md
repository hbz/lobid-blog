...
Die meiste Arbeit zur Optimierung der Datenstruktur übernimmt der JSON-LD-Kontext unter http://lobid.org/gnd/context.jsonld. Er bewirkt unter anderem folgende Dinge:

- Der Kontext bestimmt welche Keys auf welche RDF-Properties gemappt werden, so dass im JSON nicht lange URIs als Keys angezeigt werden.
...
- Mittels `"@container": "@set"`) wird festgelegt, dass bis auf wenige Ausnahmen alle Properties einen Array als Wert haben, auch wenn es nur ein Element als Wert gibt. Dadurch ist die Datenstruktur homogener und vorhersagbarer.
...
## Homogenisierung Typen und Label-Properties

Damit das bei jeder GND-Entität funktioniert, muss allen Entitäten der Typ `AuthorityResource` zugewiesen sein. Hier kommen wir zum Punkt, wo wir die Daten der DNB ergänzen, um bestimmte Funktionalitäten zu ermöglichen. Das von der DNB gelieferte [RDF zum Eintrag von Hannah Arendt](https://d-nb.info/gnd/11850391X/about/lds) enthält folgende Informationen (in Turtle Notation:
...
Dies ermöglicht es API-Nutzer/innen auf einfache Weise menschenlesbare Labels in Anwendungsoberflächen anzuzeigen anstatt bloße URIs.

## Anreicherung mit Links und Bildern aus EntityFacts

Neben dem GND-RDF stellt die DNB mit [EntityFacts](http://www.dnb.de/DE/Service/DigitaleDienste/EntityFacts/entityfacts_node.html) einen Dienst bereit, der einfaches JSON-LD zu Personen, Körperschaften und Geographika aus der GND anbietet angereichert um Links zu anderen Datenanbietern (Wikidata, ORCID, BnF etc.) sowie zu Abbildungen einer GND-Entität auf Wikimedia Commons. Die Bereitstellung eines [Dumps](https://twitter.com/junicatalo/status/976086235541397504) EntityFacts-Daten seitens der DNB hat uns dazu ermutigt, diese zusätzlichen Informationen zu ergänzen.<sup>4</sup> Im Beispiel "Hannah Arendt" sind dies unter anderem folgende Informationen:
...
Mit diesen Anreicherungen kann nun auf der Basis von Identifikatoren Dritter in lobid-gnd gesucht werden, etwa anhand einer [ORCID](http://lobid.org/gnd/search?q=%220000-0002-7613-4123%22) oder eines [ISNI](http://lobid.org/gnd/search?q=%220000000114476112%22) und es können mit den Bildern Einträge wie z. B. Autorenseiten illustriert werden.<sup>5</sup>

# Web-API

Das im vorherigen Abschnitt beschriebene JSON-LD indexieren wir in einen [Elasticsearch](https://www.elastic.co/de/products/elasticsearch)-Suchmaschinenindex und bieten die native Elasticsearch-API als Schnittstelle an. Somit sind automatisch alle nützlichen Elasticsearch-Funktionen für interessierte Nutzer verfügbar wie z.B. Unterstützung der [Lucene Query Language](https://lucene.apache.org/core/2_9_4/queryparsersyntax.html), exists-Abfragen, Elasticsearch-[Filter](https://www.elastic.co/guide/en/elasticsearch/reference/current/query-filter-context.html) und [-Aggregationen](https://www.elastic.co/guide/en/elasticsearch/reference/current/search-aggregations.html). Eine Dokumentation der Elasticsearch query_string DSL findet sich [hier](https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl-query-string-query.html). Darauf aufsetzend bieten wir auch einen [Parameter für Auto-Suggest](http://lobid.org/gnd/api#auto-complete), dessen Rückgabefelder sogar wenn erwünscht angepasst werden können.

Für eine detaillierte API-Beschreibung verweisen wir auf die in der Entstehung befindliche Dokumentation unter <a href="http://lobid.org/gnd/api">http://lobid.org/gnd/api</a>. Im Folgenden seien nur ein paar Beispiel-Abfragen genannt, wobei durch Ergänzung von `format=json` auch im Browser JSON-LD angezeigt werden kann:
...
- ["Kleinräumige Geografika" innerhalb von Berlin](http://lobid.org/gnd/search?q=geographicAreaCode.id%3A%22http%3A%2F%2Fd-nb.info%2Fstandards%2Fvocab%2Fgnd%2Fgeographic-area-code%23XA-DE-BE%22&filter=%2B%28type%3ANameOfSmallGeographicUnitLyingWithinAnotherGeographicUnit%29&size=100)


# Rückmeldungen erwünscht

lobid-gnd möchte die GND als ["Linked Open Usable Data" (LOUD)](https://www.slideshare.net/azaroth42/europeanatech-keynote-shout-it-out-loud) anbieten, so dass die Daten für Anwendungsentwickler/innen einfach zu nutzen sind und gleichzeitig eine intuitive Oberfläche für Endnutzer/innen anbietet. Wir freuen uns über jegliche Rückmeldungen, seien es Verbesserungsvorschläge, Bug-Meldungen, Lobhudeleien oder Verrisse. Bitte teilen Sie uns über Anmerkungen mit hypothes.is, per E-Mail oder Twitter mit, wie sie die Daten gerne nutzen würden, welche Funktionen Ihnen fehlen oder wo Sie einen Bug entdeckt haben. 
...
<sup>3</sup> Für Details siehe etwa die Tickets [#1](https://github.com/hbz/lobid-gnd/issues/1) und [#101](https://github.com/hbz/lobid-gnd/issues/101).

<sup>4</sup> Für Details zur Umsetzung siehe [Ticket #69](https://github.com/hbz/lobid-gnd/issues/69).

<sup>5</sup> Wobei zu beachten ist, dass die Attributions- und Lizenzinformationen zu den Bildern nicht mitgeliefert werden, sondern von der Wikimedia Commons API geladen werden müssen. Zur Umsetzung siehe z.B. [diesen Kommentar](https://github.com/hbz/lobid-organisations/issues/321#issuecomment-285366696).